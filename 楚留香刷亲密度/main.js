...
if (dialogs.singleChoice("君欲何为？", ["开始刷亲密度", "更改设置"])) {
    let endSetup = 0;
...
var i = dialogs.input("输入今天已刷次数，如果之前没刷，请填写1", "1") - 1
//繁体转换为简体
...
//用户设置
function runSetup() {
    switch (dialogs.singleChoice("请选择一个设置，所有设置都会自动保存", ["查看当前设置", "更改消息模板", "更改停止次数", "调整发送速度", "调整点击位置", "恢复默认设置", "查看获取坐标方法"])) {
        case 0:
        ...
        case 2:
            ...
            break;
        case 3:
            setConfigSafe("sendDelay", dialogs.input("输入两次发送之间的延迟，单位:毫秒，越低发送越快但是太低了会被系统拦截，如果对方在线建议1000，对方不在线建议4700。", "4700"));
            break;
        case 4:
...
//获取一句诗词
function getPoem() {
    var files_ = files.listDir("./gushi_json/");
    var targetFileName = files_[Math.floor(Math.random() * files_.length)];
    //log(file_);
    var pstr = files.read("./gushi_json/" + targetFileName);
...
var sendDelay = config.get("sendDelay", 450);
...
while (i < maxCount) {
    ...
    className("android.widget.EditText").findOne().setText(msg);
