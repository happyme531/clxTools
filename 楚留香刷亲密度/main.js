...
function runSetup() {
    switch (dialogs.singleChoice("请选择一个设置，所有设置都会自动保存", ["查看当前设置", "更改消息模板", "更改停止次数", "调整发送速度", "调整点击位置", "切换自动重试", "恢复默认设置", "发送完成后操作", "查看获取坐标方法"])) {
        case 0: //查看当前设置
            ...
            break;
        case 1: //更改消息模板
            setConfigSafe("msgTemplate",
                dialogs.rawInput("输入你想发送的消息，$n代表当前发送的条数，$t代表当前时间，$p代表一句诗词，$r代表一句自定义内容",
                    config.get("msgTemplate", "当前是$t,第$n次，给你一句诗词:$p")));
            break;
        case 2: //更改停止次数
        ...
        let curPosStr = ""
            try{
            let curPos = config.get("clickPos",[[0,0],[0,0]]);
            curPosStr =  "[[" + curPos[0][0] + "," + curPos[0][1] + "],[" +  curPos[1][0] + "," + curPos[1][1] + "]]"; 
            }catch(e){
                config.put("clickPos", [[0,0],[0,0]]);
                curPosStr = "[[0,0],[0,0]]";
            }
            config.put("clickPos", 
              dialogs.input("按\"[[点击输入文字的方框正中间x坐标,y坐标],[发送按钮正中间的x坐标,y坐标]]\"格式输入坐标，例如: [[1618,1173],[2197,1170]]", 
              curPosStr));
            break;
        case 5: //切换自动重试
        ...
        case 6: //恢复默认设置
            ...
            break;
        case 7: //发送完成后操作
            setConfigSafe("endingInstruction", dialogs.singleChoice("选择发送完成后的操作..", ["无操作", "关闭屏幕(需要root权限)"]));
            break;
        case 8: //查看获取坐标方法
...
function getPoem(logEnable) {
    let files_ = files.listDir("./gushi_json/", function(name) {
        return name.endsWith(".json") && files.isFile(files.join("./gushi_json/", name));
    });
...
function sendMessage(msg, timeOut) {
    ...
    if (operatingMode == 0) {
        ...
        //不知为何，RootAutomator用不了
        Tap(clickPos[0][0], clickPos[0][1]);
    };
    sleep(400);
...
//检测root权限
var deviceHasRoot = 1;
if (shell("cat /proc/cmdline 1>/dev/null", true).error) deviceHasRoot = 0;
...
if (screenOffEnabled && !deviceHasRoot) {
    console.error("检测不到root权限!请关闭自动关屏设定");
    toast("检测不到root权限!请关闭自动关屏设定");
    exit();
};
if (screenOffEnabled) var sh = new Shell(true);
...
toast("脚本将在几秒后开始运行")
if (autoRetryEnabled) sleep(5000);
startSending();
